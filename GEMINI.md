...
## ドキュメントと情報源
- **開発ドキュメントサイト**: `docs-site/` に Docusaurus ベースのサイトがあります。
...
### 2. UI 実装方針
- **新規 MUI 依存の禁止**: 新規に MUI コンポーネントを導入せず、可能な限り Tailwind CSS + 標準 HTML 要素で実装してください。
- **既存 MUI の移行**: 既存の MUI 箇所を修正する場合は、少しずつ Tailwind CSS ベースへ置き換えることを検討してください。
- **デザイントークン**: スタイリングには `src/shared/designSystem/` のトークンを使用し、`designTokenVar()` を介して CSS 変数を参照してください。
