...
export default function OperationLogUpdateForm(props) {
  const {
    id: idProp,
    operationLog: operationLogModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = operationLogRecord
      ? { ...initialValues, ...operationLogRecord }
      : initialValues;
    ...
    setClientTimezone(cleanValues.clientTimezone);
    setOccurredAt(cleanValues.occurredAt);
    setResolvedWorkDate(cleanValues.resolvedWorkDate);
    setIdempotencyKey(cleanValues.idempotencyKey);
    setAppVersion(cleanValues.appVersion);
    setSeverity(cleanValues.severity);
    setVersion(cleanValues.version);
    setErrors({});
  };
  const [operationLogRecord, setOperationLogRecord] = React.useState(
    operationLogModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
          ...
          )?.data?.getOperationLog
        : operationLogModelProp;
      setOperationLogRecord(record);
    };
    queryData();
  }, [idProp, operationLogModelProp]);
  React.useEffect(resetStateValues, [operationLogRecord]);
  const validations = {
    staffId: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          resolvedWorkDate: resolvedWorkDate ?? null,
      ...
      {...getOverrideProps(overrides, "OperationLogUpdateForm")}
      {...rest}
    >
      <TextField
        label="Staff id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || operationLogModelProp)}
