...
import { getWorkflowNotificationEvent } from "../shared/api/graphql/documents/queries";
import { updateWorkflowNotificationEvent } from "../shared/api/graphql/documents/mutations";
const client = generateClient();
export default function WorkflowNotificationEventUpdateForm(props) {
  const {
    id: idProp,
    workflowNotificationEvent: workflowNotificationEventModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = workflowNotificationEventRecord
      ? { ...initialValues, ...workflowNotificationEventRecord }
      : initialValues;
    setRecipientStaffId(cleanValues.recipientStaffId);
    setActorStaffId(cleanValues.actorStaffId);
    setWorkflowId(cleanValues.workflowId);
  ...
  const [workflowNotificationEventRecord, setWorkflowNotificationEventRecord] =
    React.useState(workflowNotificationEventModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getWorkflowNotificationEvent.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWorkflowNotificationEvent
        : workflowNotificationEventModelProp;
      setWorkflowNotificationEventRecord(record);
    };
    queryData();
  }, [idProp, workflowNotificationEventModelProp]);
  React.useEffect(resetStateValues, [workflowNotificationEventRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateWorkflowNotificationEvent.replaceAll("__typename", ""),
            variables: {
              input: {
                id: workflowNotificationEventRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WorkflowNotificationEventUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workflowNotificationEventModelProp)}
