...
import { formatDateRangeLabel, getAttendanceQueryDateRange, getCurrentMonthFromQuery, getEffectiveDateRange, MONTH_QUERY_KEY, shouldRefetchForAttendanceEvent, } from "./attendanceListUtils";
...
export default function AttendanceTable() {
    ...
    const currentMonth = useMemo(() => getCurrentMonthFromQuery(searchParams.get(MONTH_QUERY_KEY)), [searchParams]);
    const handleMonthChange = useCallback((nextMonth: Dayjs) => {
    ...
    const endDate = attendanceQueryDateRange.end.format(AttendanceDate.DataFormat);
    const { data: attendances = [], isLoading: isAttendancesInitialLoading, isFetching: isAttendancesFetching, isUninitialized: isAttendancesUninitialized, error: attendancesError, refetch: refetchAttendances, } = useListAttendancesByDateRangeQuery({
    ...
    useEffect(() => {
        ...
        if (!currentStaffId || !shouldFetchAttendances)
            return;
        ...
        const createSubscription = graphqlClient
            ...
            next: ({ data }: {
                ...
                if (!shouldRefetchForAttendanceEvent(currentStaffId, queryRange, attendance?.staffId, attendance?.workDate)) {
                    return;
                }
                scheduleRefetch();
            },
    ...
    const logger = useMemo(() => new Logger("AttendanceList", import.meta.env.DEV ? "DEBUG" : "ERROR"), []);
    ...
    const rangeLabelForDisplay = useMemo(() => formatDateRangeLabel(effectiveDateRange), [effectiveDateRange]);
