...
function getBadgeContent(attendances: Attendance[]) {
  return attendances.reduce((total, attendance) => {
    return (
      total + new ChangeRequest(attendance.changeRequests).getUnapprovedCount()
    );
  }, 0);
}

function hasAnyUnapprovedChangeRequest(attendances: Attendance[]) {
...
function AttendanceTotalStatus({
  ...
  if (hasAnyUnapprovedChangeRequest(attendances)) {
    return (
