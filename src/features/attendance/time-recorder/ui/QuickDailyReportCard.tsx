 ...
 */
const extractErrorMessage = (errors: readonly {
    message: string;
}[]): string => {
...
export default function QuickDailyReportCard({ staffId, date, }: QuickDailyReportCardProps) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const report = response.data?.dailyReportsByStaffId?.items?.filter((item): item is NonNullable<typeof item> => Boolean(item))[0] ?? null;
                if (!mounted)
                    return;
                if (report) {
    ...
    return (<QuickDailyReportCardView date={date} reportId={reportId} content={content} isOpen={isOpen} isDialogOpen={isDialogOpen} isLoading={isLoading} isEditable={isEditable} isSaving={isSaving} hasStaff={hasStaff} error={error} lastSavedAt={lastSavedAt} contentPanelId={contentPanelId} isSubmitted={reportStatus === DailyReportStatus.SUBMITTED} onToggle={handleToggle} onDialogOpen={handleDialogOpen} onDialogClose={handleDialogClose} onSave={() => void handleSave(true, true)} onContentChange={(value) => setContent(value)}/>);
}
