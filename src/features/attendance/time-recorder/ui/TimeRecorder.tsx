...
import { clockInAction, clockOutAction, GoDirectlyFlag, restEndAction, restStartAction, ReturnDirectlyFlag, } from "@/entities/attendance/lib/actions/attendanceActions";
...
import { type TimeRecorderContextValue, TimeRecorderProvider, } from "./TimeRecorderContext";
import { formatClockDisplayText, hasPendingChangeRequests, resolveElapsedWorkInfo, summarizeAttendanceErrors, type TimeRecorderElapsedWorkInfo, toConfiguredTimeISO, } from "./timeRecorderUtils";
...
export default function TimeRecorder({ onAttendanceErrorCountChange, onElapsedWorkTimeChange, }: TimeRecorderProps): JSX.Element {
    ...
    const attendanceLoading = !shouldFetchAttendance ||
        isAttendanceInitialLoading ||
        isAttendanceFetching ||
        isAttendanceUninitialized;
    const attendancesLoading = !shouldFetchAttendance ||
    ...
    const updateAttendance = useCallback((input: Parameters<typeof updateAttendanceMutation>[0]) => {
    ...
    const runAttendanceActionWithRefresh = useCallback(async <T,>(action: () => Promise<T>) => {
        const result = await action();
        await refreshAttendanceData();
        return result;
    }, [refreshAttendanceData]);
    const clockIn = useCallback((staffId: string, workDate: string, startTime: string, goDirectlyFlag = GoDirectlyFlag.NO) => runAttendanceActionWithRefresh(() => clockInAction({
        attendance,
        staffId,
        workDate,
        startTime,
    ...
    const clockOut = useCallback((staffId: string, workDate: string, endTime: string, returnDirectlyFlag = ReturnDirectlyFlag.NO) => runAttendanceActionWithRefresh(() => clockOutAction({
        attendance,
        staffId,
        workDate,
        endTime,
    ...
    const restStart = useCallback((staffId: string, workDate: string, startTime: string) => runAttendanceActionWithRefresh(() => restStartAction({
        attendance,
        staffId,
        workDate,
        time: startTime,
    ...
    const restEnd = useCallback((staffId: string, workDate: string, endTime: string) => runAttendanceActionWithRefresh(() => restEndAction({
        attendance,
        staffId,
        workDate,
        time: endTime,
    ...
    const [workStatus, setWorkStatus] = useState<WorkStatus | null | undefined>(undefined);
    ...
    // 変更リクエスト中かどうか
    const hasChangeRequest = useMemo(() => hasPendingChangeRequests(attendance), [attendance]);
    ...
    const handleClockIn = useCallback(() => {
        ...
        return clockInCallback(cognitoUser, clockIn, dispatch, staff, logger, occurredAt);
    }, [clockIn, cognitoUser, dispatch, logger, staff]);
    const handleClockOut = useCallback(() => {
        const occurredAt = getNowISOStringWithZeroSeconds();
        return clockOutCallback(cognitoUser, clockOut, dispatch, staff, logger, undefined, occurredAt);
    ...
    const handleGoDirectly = useCallback(() => {
        ...
        return goDirectlyCallback(cognitoUser, staff, dispatch, clockIn, logger, startIso, occurredAt);
    ...
    const handleReturnDirectly = useCallback(() => {
        ...
        return returnDirectlyCallback(cognitoUser, staff, dispatch, clockOut, logger, endIso, occurredAt);
    }, [cognitoUser, staff, dispatch, clockOut, logger, getEndTime]);
    const handleRestStart = useCallback(() => {
        const occurredAt = getNowISOStringWithZeroSeconds();
        return restStartCallback(cognitoUser, dispatch, restStart, logger, occurredAt);
    ...
    const handleVisibilityChange = useCallback(() => {
        ...
        if (now.diff(lastActiveTimeRef.current, "minute") >
    ...
    useEffect(() => {
        ...
        const subscription = graphqlClient
            ...
            next: (event) => {
                ...
                const shouldIgnoreLocalUpdate = Date.now() < localAttendanceUpdateIgnoreUntilRef.current;
    ...
    return (<TimeRecorderProvider value={contextValue!}>
      <TimeRecorderView />
