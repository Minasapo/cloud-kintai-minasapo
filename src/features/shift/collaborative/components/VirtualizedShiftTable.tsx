    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          ...
          {isAdmin &&
            (() => {
              const nonEmptyCells = Array.from(staffData.values()).filter(
                (c) => c.state !== "empty",
              );
              const allLocked =
                nonEmptyCells.length > 0 && nonEmptyCells.every((c) => c.isLocked);
              return (
                <Button
                  size="small"
                  variant="outlined"
...
export const VirtualizedShiftTable = memo<VirtualizedShiftTableProps>(
  ({
    ...
    const confirmDialogProps = buildConfirmDialogProps(pendingAction, currentMonth);
            ...
            {staffIds.map((staffId) => {
              ...
              return (
                <StaffTableRow
                  ...
                  onRequestPendingAction={setPendingAction}
                />
              );
            })}
