...
export function useShiftRequestData({ cognitoUserId, monthStart, }: UseShiftRequestDataParams): UseShiftRequestDataResult {
    ...
    const targetMonthKey = useMemo(() => monthStart.format("YYYY-MM"), [monthStart]);
    ...
    useEffect(() => {
        ...
        const fetchShiftRequest = async () => {
            ...
            try {
                ...
                const items = response.data?.shiftRequestsByStaffId?.items?.filter((item): item is NonNullable<typeof item> => item !== null) ?? [];
                ...
                const sanitizedHistories = existing.histories
                ...
                setHistories(sanitizedHistories.toSorted((a, b) => (a.version ?? 0) - (b.version ?? 0)));
            }
            catch {
                if (isMounted) {
                    dispatch(pushNotification({
                        tone: "error",
                        message: MESSAGE_CODE.E16002
                    }));
                    resetShiftRequestState();
                }
            }
