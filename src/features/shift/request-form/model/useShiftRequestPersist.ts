...
export function useShiftRequestPersist({ staff, monthStart, selectedDates, note, histories, shiftRequestId, setShiftRequestId, setHistories, }: UseShiftRequestPersistParams) {
    const dispatch = useDispatch();
    const [isSaving, setIsSaving] = useState(false);
    const saveShiftRequest = useCallback(async (summary: Summary) => {
        if (!staff?.id) {
        ...
        const maxVersion = histories.reduce((acc, history) => Math.max(acc, history.version ?? 0), 0);
        ...
        try {
            ...
            if (shiftRequestId) {
                ...
                setShiftRequestId(response.data?.updateShiftRequest?.id ?? shiftRequestId);
