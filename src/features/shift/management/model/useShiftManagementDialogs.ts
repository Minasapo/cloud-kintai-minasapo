...
export type ApplyShiftStateFn = (staffIds: string[], dayKeys: string[], nextState: ShiftState) => Promise<void>;
export default function useShiftManagementDialogs(applyShiftState: ApplyShiftStateFn) {
    const dispatch = useDispatch();
    const [editingCell, setEditingCell] = useState<ShiftEditingTarget | null>(null);
    ...
    const openShiftEditDialog = useCallback((target: ShiftEditingTarget, state: ShiftState) => {
    ...
    const applyBulkEdit = useCallback(async (staffIds: string[], dayKeys: string[]) => {
        if (!staffIds.length || !dayKeys.length)
            return;
