...
import { roleLabelMap, StaffRole, useStaffs, } from "@entities/staff/model/useStaffs/useStaffs";
...
import { CreateStaffDialog, MoreActionButton, SyncCognitoUser, } from "@/features/admin/staff/ui/actions";
...
export default function AdminStaff() {
    ...
    const { staffs, loading: staffLoading, error: staffError, refreshStaff, createStaff, updateStaff, deleteStaff, } = useStaffs({ isAuthenticated });
    const [searchQuery, setSearchQuery] = useState("");
    const normalizedSearchQuery = searchQuery.trim().toLowerCase();
    const filteredStaffs = useMemo(() => staffs
        .filter((staff) => {
    ...
    const passwordChangeRequiredCount = staffs.filter((staff) => staff.status === "FORCE_CHANGE_PASSWORD").length;
    if (staffLoading) {
        return (<div className="w-full px-2 pt-2 sm:px-4 md:px-6">
      ...
      <div className="space-y-2.5">
        <section className="admin-staff-hero">
          <div className="flex flex-col items-stretch gap-1 sm:flex-row sm:justify-end sm:items-center">
            <CreateStaffDialog staffs={staffs} refreshStaff={refreshStaff} createStaff={createStaff} updateStaff={updateStaff}/>
            <SyncCognitoUser staffs={staffs} refreshStaff={refreshStaff} createStaff={createStaff} updateStaff={updateStaff}/>
          </div>
        </section>
        ...
        <section className="admin-staff-toolbar">
          <div className="w-full space-y-1.5">
            ...
            <label className="relative block">
              ...
              <input className="w-full rounded-md border border-slate-300 bg-white py-2 pl-9 pr-3 text-sm outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100" placeholder="スタッフ名・スタッフID・メールで検索" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
            </label>
        ...
        <section className="admin-staff-table-container overflow-auto">
          <table className="min-w-[1180px] text-sm">
            ...
            <tbody>
              {filteredStaffs.length === 0 ? (<tr>
                  <td colSpan={11} className="admin-staff-empty">
                    条件に一致するスタッフが見つかりません。
                  </td>
                </tr>) : (filteredStaffs.map((staff) => {
            const fullName = `${staff.familyName ?? ""} ${staff.givenName ?? ""}`.trim();
            ...
            const attendanceManaged = staff.attendanceManagementEnabled !== false;
            const workTypeLabel = getWorkTypeLabel((staff as unknown as Record<string, unknown>).workType as string | null);
            return (<tr key={staff.id} className="admin-staff-table-row">
                      <td className="admin-staff-action-cell whitespace-nowrap px-3 py-1.5">
                        <div className="flex items-center gap-0.5">
                          <EditButton staff={staff}/>
                          <MoreActionButton staff={staff} updateStaff={updateStaff} deleteStaff={deleteStaff}/>
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-4 py-2">
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${staff.enabled
                    ...
                    : "border-slate-300 bg-white text-slate-600"}`}>
                      ...
                      <td className="whitespace-nowrap px-4 py-2">
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${staff.status === "CONFIRMED"
                    ? "border-teal-200 bg-teal-50 text-teal-700"
                    : "border-amber-300 bg-amber-50 text-amber-800"}`}>
                      ...
                      <td className="whitespace-nowrap px-4 py-2">
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${attendanceManaged
                    ? "border-teal-200 bg-teal-50 text-teal-700"
                    : "border-slate-300 bg-white text-slate-600"}`}>
