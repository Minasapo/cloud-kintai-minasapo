...
export default function CreateStaffDialog({ staffs, refreshStaff, createStaff, updateStaff, }: {
    staffs: StaffType[];
    ...
    const onSubmit = async (data: Inputs) => {
        ...
        try {
            await handleSyncCognitoUser(staffs, refreshStaff, createStaff, updateStaff);
        }
        catch {
            dispatch(pushNotification({
                tone: "error",
        ...
        try {
            ...
            const targetStaff = latestStaffs.find((staff) => (staff.mailAddress ?? "").toLowerCase() ===
    ...
    return (<>
      <button type="button" className="inline-flex h-9 items-center justify-center rounded-xl border border-emerald-700/15 bg-[#19b985] px-4 text-xs font-semibold leading-none tracking-wide text-white shadow-[0_10px_20px_-14px_rgba(5,150,105,0.75)] transition hover:bg-[#17ab7b] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200" onClick={handleClickOpen}>
      ...
      {dialog}
      {open ? (<div className="fixed inset-0 z-[1400] flex items-center justify-center bg-slate-900/35 p-4" onClick={requestClose}>
          <div className="w-full max-w-5xl rounded-2xl bg-slate-50 p-3 shadow-2xl sm:p-4" onClick={(e) => e.stopPropagation()}>
            <div className="space-y-2.5">
                ...
                <table className="w-full min-w-[860px]">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className={VALUE_CELL_CLASS}>
                        <Controller name="attendanceManagementEnabled" control={control} render={({ field }) => (<div className="space-y-1">
                              <Switch checked={field.value ?? true} onChange={(e) => {
                    setValue("attendanceManagementEnabled", e.target.checked, {
                        shouldDirty: true,
                        shouldValidate: true,
                    });
                    ...
                    {cognitoUser?.owner && (<tr>
                        ...
                        <td className={VALUE_CELL_CLASS}>
                          <Controller name="developer" control={control} render={({ field }) => (<Switch checked={Boolean(field.value)} onChange={() => {
              ...
              <div className="flex justify-end gap-2 pb-1 pt-1">
                <Button type="button" variant="text" color="inherit" onClick={requestClose}>
                  キャンセル
                </Button>
                <Button type="button" variant="contained" disabled={!isDirty || !isValid || isSubmitting} startIcon={isSubmitting ? <CircularProgress size={15}/> : undefined} onClick={handleSubmit(onSubmit)}>
                  登録
                </Button>
              </div>
            </div>
