import { StaffRole, StaffType, useStaffs, } from "@entities/staff/model/useStaffs/useStaffs";
import { Autocomplete, Button, Chip, CircularProgress, FormControlLabel, LinearProgress, Radio, RadioGroup, Switch, Tab, Tabs, TextField, } from "@mui/material";
...
const LABEL_CELL_CLASS = "w-[220px] min-w-[180px] border-b border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-900";
...
export default function AdminStaffEditor() {
    ...
    const { staffs, loading: staffLoading, error: staffError, updateStaff, } = useStaffs({ isAuthenticated });
    const { register, control, setValue, watch, getValues, handleSubmit, reset, formState: { isValid, isDirty, isSubmitting }, } = useForm<Inputs>({
        mode: "onChange",
        defaultValues: {
            owner: false,
            developer: false,
    ...
    useEffect(() => {
        ...
        setValue("attendanceManagementEnabled", extendedStaff.attendanceManagementEnabled ?? true);
        setValue("role", staff.role ?? null);
        setValue("developer", extendedStaff.developer ?? false);
    }, [staffId, staffs, setValue]);
    const shiftGroupOptions = useMemo(() => getShiftGroups().map((group) => ({
        value: group.label,
        label: group.label,
    ...
    return (<div className="h-full w-full px-2 pb-3 pt-2 sm:px-4 md:px-6">
      {dialog}
      <div className="space-y-2.5">
        ...
        <section className="rounded-2xl border border-emerald-100 bg-white/95 px-4 pt-2">
          <Tabs value={tabIndex} onChange={(_, v) => setTabIndex(v)} aria-label="スタッフ編集タブ">
            <Tab label="全般"/>
            <Tab label="高度設定" disabled={!cognitoUser?.owner} data-testid="advanced-tab"/>
          </Tabs>
        </section>
          ...
          <table className="w-full min-w-[860px]">
            <tbody>
              {tabIndex === 0 && (<>
                  ...
                  {cognitoUser?.owner && (<tr>
                      <td className={LABEL_CELL_CLASS}>オーナー権限</td>
                      <td className={VALUE_CELL_CLASS}>
                        <Controller name="owner" control={control} render={({ field }) => (<Switch checked={field.value} onChange={() => {
                  ...
                  <tr>
                    ...
                    <td className={VALUE_CELL_CLASS}>
                      <Controller name="usageStartDate" control={control} render={({ field }) => (<DatePicker value={field.value ? dayjs(field.value) : null} onChange={(v) => {
                    const next = v ? v.format("YYYY-MM-DD") : null;
                    field.onChange(next);
                }} format="YYYY/M/D" slotProps={{
                    textField: {
                        onBlur: field.onBlur,
                        size: "small",
                    },
                  ...
                  <tr>
                    ...
                    <td className={VALUE_CELL_CLASS}>
                      <Controller name="attendanceManagementEnabled" control={control} render={({ field }) => (<div className="space-y-1">
                            <Switch checked={field.value ?? true} onChange={(e) => field.onChange(e.target.checked)}/>
                  ...
                  <tr>
                    ...
                    <td className={VALUE_CELL_CLASS}>
                      <Controller name="workType" control={control} render={({ field }) => (<Autocomplete {...field} value={WORK_TYPE_OPTIONS.find((option) => option.value === field.value) ?? null} options={WORK_TYPE_OPTIONS} getOptionLabel={(option) => option.label} renderInput={(params) => (<TextField {...params} size="small" sx={{ width: { xs: "100%", sm: 400 } }}/>)} onChange={(_, data) => {
                    if (!data)
                        return;
                  ...
                  <tr>
                    ...
                    <td className={VALUE_CELL_CLASS}>
                      {shiftGroupOptions.length === 0 ? (<p className="text-sm text-slate-500">
                          利用可能なシフトグループがありません。管理画面の「シフト設定」で登録してください。
                        </p>) : (<Controller name="shiftGroup" control={control} render={({ field }) => {
                    const selectedOption = shiftGroupOptions.find((option) => option.value === field.value) ?? null;
                    return (<Autocomplete value={selectedOption} options={shiftGroupOptions} onChange={(_, newValue) => {
                            field.onChange(newValue?.value ?? null);
                        }} isOptionEqualToValue={(option, value) => option.value === value.value} renderInput={(params) => (<TextField {...params} size="small" sx={{ width: { xs: "100%", sm: 400 } }} placeholder="所属させるシフトグループを選択" onBlur={field.onBlur}/>)}/>);
                }}/>)}
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td className={VALUE_CELL_CLASS}>
                      <Controller name="approverSetting" control={control} render={({ field }) => (<RadioGroup row value={field.value} onChange={(e) => {
                  ...
                  <ApproverSettingTableRows control={control} watch={watch} staffs={staffs} currentCognitoUserId={cognitoUser?.id}/>
                </>)}

              {tabIndex === 1 && (<tr>
                  <td className={LABEL_CELL_CLASS}>開発者フラグ</td>
                  <td className="px-4 py-3 align-middle">
                    <Controller name="developer" control={control} render={({ field }) => (<Switch data-testid="developer-flag-checkbox" checked={Boolean(field.value)} onChange={() => {
        ...
        <div className="flex justify-end pb-8 pt-2">
          <Button data-testid="save-button" variant="contained" size="medium" disabled={!isValid || !isDirty || saving || isSubmitting} startIcon={saving ? <CircularProgress size={15}/> : undefined} onClick={handleSubmit(onSubmit)}>
            保存
          </Button>
        </div>
      </div>
...
function ApproverSettingTableRows({ control, watch, staffs, currentCognitoUserId, }: {
    ...
    const adminOptions = useMemo<AutocompleteOption[]>(() => {
        return staffs
            .filter((staff) => (staff.role === StaffRole.ADMIN || staff.owner) &&
            staff.cognitoUserId !== currentCognitoUserId)
            .map((staff) => ({
            value: staff.cognitoUserId ?? "",
            label: [staff.familyName, staff.givenName]
                .filter((n) => Boolean(n))
                .join(" ") ||
                staff.mailAddress ||
                "",
    ...
    return (<>
      {approverSetting === ApproverSettingMode.SINGLE && (<tr>
          <td className={LABEL_CELL_CLASS}/>
          <td className={VALUE_CELL_CLASS}>
            <Controller name="approverSingle" control={control} rules={{
                validate: (value) => {
                    if (approverSetting !== ApproverSettingMode.SINGLE)
                        return true;
                ...
                const valueOption = adminOptions.find((option) => option.value === field.value);
                return (<Autocomplete value={valueOption ?? null} options={adminOptions} onChange={(_, newValue) => {
                        field.onChange(newValue?.value ?? null);
                    }} getOptionLabel={(option) => option.label} isOptionEqualToValue={(option, value) => option.value === value.value} renderInput={(params) => (<TextField {...params} size="small" sx={{ width: { xs: "100%", sm: 400 } }} label="承認者" placeholder="承認者を検索" error={Boolean(fieldState.error)} helperText={fieldState.error?.message ||
                            "承認者を1名選択してください。"} onBlur={field.onBlur}/>)}/>);
            }}/>
          </td>
        </tr>)}
      ...
      {approverSetting === ApproverSettingMode.MULTIPLE && (<>
          <tr>
            <td className={LABEL_CELL_CLASS}/>
            <td className={VALUE_CELL_CLASS}>
              <Controller name="approverMultiple" control={control} rules={{
                validate: (value) => {
                    if (approverSetting !== ApproverSettingMode.MULTIPLE)
                        return true;
                ...
                return (<Autocomplete<AutocompleteOption, true> multiple options={adminOptions} value={valueOptions} onChange={(_, newValue) => {
                        field.onChange(newValue.map((option) => option.value));
                    }} disableCloseOnSelect getOptionLabel={(option) => option.label} isOptionEqualToValue={(option, value) => option.value === value.value} renderTags={(tagValue, getTagProps) => tagValue.map((option, index) => (<Chip {...getTagProps({ index })} key={option.value} label={option.label} size="small"/>))} renderInput={(params) => (<TextField {...params} size="small" sx={{ width: { xs: "100%", sm: 400 } }} label="承認者" placeholder="承認者を選択" error={Boolean(fieldState.error)} helperText={fieldState.error?.message ||
                            (approverMultipleMode === ApproverMultipleMode.ORDER
                                ? "選択した順番が承認順になります。"
                                : "複数選択できます。")} onBlur={field.onBlur}/>)}/>);
            }}/>
            </td>
          </tr>
          ...
          <tr>
            <td className={LABEL_CELL_CLASS}/>
            <td className={VALUE_CELL_CLASS}>
              <Controller name="approverMultipleMode" control={control} render={({ field }) => (<RadioGroup row value={field.value ?? ApproverMultipleMode.ANY} onChange={(event) => {
                    const nextValue = event.target
                        .value as ApproverMultipleMode;
                    field.onChange(nextValue);
                }}>
                    <FormControlLabel value={ApproverMultipleMode.ANY} control={<Radio />} label="誰か1人が承認すれば完了"/>
                    <FormControlLabel value={ApproverMultipleMode.ORDER} control={<Radio />} label="設定した順番で承認"/>
                  </RadioGroup>)}/>
            </td>
          </tr>

          {approverMultipleMode === ApproverMultipleMode.ORDER && (<tr>
              <td className={LABEL_CELL_CLASS}/>
              <td className={VALUE_CELL_CLASS}>
                <div className="space-y-1">
                  {selectedMultipleOptions.length === 0 ? (<p className="text-sm text-slate-500">
                      承認順を設定するスタッフを選択してください。
                    </p>) : (selectedMultipleOptions.map((option, index) => (<div key={option.value} className="flex items-center gap-2">
...
function MailAddressTableCell({ register, }: {
    register: UseFormRegister<Inputs>;
}) {
    return (<td className={VALUE_CELL_CLASS}>
      <TextField {...register("mailAddress", { required: true })} size="small" sx={{ width: { xs: "100%", sm: 400 } }}/>
    </td>);
}
