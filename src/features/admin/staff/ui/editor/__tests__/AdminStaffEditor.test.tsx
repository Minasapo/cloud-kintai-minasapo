  ...
  StaffNameTableCell: ({ register }: { register: unknown }) => (
    <td data-testid="staff-name-cell">
      <input
        ...
        {...(register as CallableFunction)("familyName", { required: true })}
        placeholder="姓"
      />
      <input
        data-testid="givenName-input"
        {...(register as CallableFunction)("givenName", { required: true })}
        placeholder="名"
      />
    </td>
  ),
...
jest.mock("@mui/material/Autocomplete", () => {
  return function MockAutocomplete({
    ...
    const currentValue = multiple
      ? Array.isArray(value)
        ? value.map((v: { value: string }) => v.value).join(",")
        : ""
...
const makeAppConfigContext = (
  shiftGroups: { label: string; description?: string }[] = [],
) => ({
  ...
  return render(
    <AuthContext.Provider value={makeAuthContext(authOverrides)}>
      <AppConfigContext.Provider
        value={makeAppConfigContext(appConfigShiftGroups) as never}
      >
        <AdminStaffEditor />
      </AppConfigContext.Provider>
...
describe("AdminStaffEditor", () => {
  ...
  describe("通常レンダリング", () => {
    ...
    it("説明テキストが表示される", () => {
      renderComponent();
      expect(
        screen.getByText("スタッフ情報と承認設定を更新できます。"),
      ).toBeInTheDocument();
    });
  ...
  describe("承認者設定ラジオボタン", () => {
    it("「管理者全員 (デフォルト)」ラジオが表示される", () => {
      renderComponent();
      expect(
        screen.getByLabelText("管理者全員 (デフォルト)"),
      ).toBeInTheDocument();
    });
    ...
    it("「特定の承認者を1名に限定」ラジオが表示される", () => {
      renderComponent();
      expect(
        screen.getByLabelText("特定の承認者を1名に限定"),
      ).toBeInTheDocument();
    });
    ...
    it("「特定の承認者を複数選択」ラジオが表示される", () => {
      renderComponent();
      expect(
        screen.getByLabelText("特定の承認者を複数選択"),
      ).toBeInTheDocument();
    });
    ...
    it("MULTIPLE を選択すると承認モードのラジオグループが表示される", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByLabelText("誰か1人が承認すれば完了"),
        ).toBeInTheDocument();
  ...
  describe("onSubmit 成功", () => {
    it("フォームを dirty にして保存するとき updateStaff が呼ばれる", async () => {
      renderComponent();
      // フォームを変更して dirty にする
      const sortKeyInput = document.querySelector(
        "input[name='sortKey']",
      ) as HTMLInputElement;
