...
type ShiftPlanDataState = {
    ...
    handleDailyCapacityChange: (month: number, dayIndex: number, value: string) => void;
    handleSaveAll: () => Promise<void>;
    performSave: (rows: ShiftPlanRow[], year: number, recordIds: Record<number, ShiftPlanRecordMeta>, showNotification?: boolean) => Promise<boolean>;
};
export const useShiftPlanData = (): ShiftPlanDataState => {
    ...
    const [yearRecordIds, setYearRecordIds] = useState<Record<number, ShiftPlanRecordMeta>>({});
    const yearRecordIdsRef = useRef<Record<number, ShiftPlanRecordMeta>>({});
    const saveQueueRef = useRef<Promise<void>>(Promise.resolve());
    const [lastAutoSaveTime, setLastAutoSaveTime] = useState<string | null>(null);
    const [savedYearlyPlans, setSavedYearlyPlans] = useState<Record<number, ShiftPlanRow[]>>(() => ({
        [initialYear]: createDefaultRows(initialYear),
    }));
    ...
    useEffect(() => {
        ...
        const fetchYearPlan = async () => {
            ...
            try {
                ...
                const record = response.data?.shiftPlanYearByTargetYear?.items?.find((item): item is NonNullable<typeof item> => item !== null) ?? null;
                if (record) {
                    setYearlyPlans((prev) => ({
                        ...prev,
    ...
    const updateYearRows = useCallback((year: number, updater: (rows: ShiftPlanRow[]) => ShiftPlanRow[]) => {
        setYearlyPlans((prev) => {
    ...
    const handleFieldChange = useCallback((month: number, field: EditableField, value: string) => {
        updateYearRows(selectedYear, (rows) => rows.map((row) => row.month === month ? { ...row, [field]: value } : row));
    }, [selectedYear, updateYearRows]);
    const handleToggleEnabled = useCallback((month: number) => {
        updateYearRows(selectedYear, (rows) => rows.map((row) => row.month === month ? { ...row, enabled: !row.enabled } : row));
    }, [selectedYear, updateYearRows]);
    const handleDailyCapacityChange = useCallback((month: number, dayIndex: number, value: string) => {
        const normalizedValue = sanitizeCapacityValue(value);
        updateYearRows(selectedYear, (rows) => rows.map((row) => {
            if (row.month !== month)
                return row;
    ...
    const performSave = useCallback(async (rows: ShiftPlanRow[], year: number, recordIds: Record<number, ShiftPlanRecordMeta>, showNotification = true) => {
                ...
                if (existingRecord) {
                    const response = (await graphqlClient.graphql({
                        ...
                        variables: {
                            ...
                            condition: buildVersionOrUpdatedAtCondition(existingRecord.version, existingRecord.updatedAt),
