...
const sanitizeRests = (
  rests?: Array<{ startTime?: string | null; endTime?: string | null } | null> | null
): RestInput[] =>
...
const sanitizeHourlyPaidHolidayTimes = (
  hourlyTimes?: Array<{ startTime?: string | null; endTime?: string | null } | null> | null
): HourlyPaidHolidayTimeInput[] =>
  hourlyTimes
    ?.filter(nonNullable)
  ...
  endpoints: (builder) => ({
    ...
    >({
      ...
      providesTags: (result, _error, arg) => {
        if (!result) {
          return [
            { type: "Attendance" as const, id: buildAttendanceCacheId(arg.staffId, arg.workDate) },
          ];
        }

        return [
          { type: "Attendance" as const, id: result.id || buildAttendanceCacheId(arg.staffId, arg.workDate) },
        ];
      },
    }),
    listRecentAttendances: builder.query<Attendance[], { staffId: string; days?: number }>({
      async queryFn({ staffId, days = 30 }, _queryApi, _extraOptions, baseQuery) {
      ...
      providesTags: (result) => {
        ...
        return [
          ...
          ...result.map((attendance) => ({
            ...
            id: attendance.id || buildAttendanceCacheId(attendance.staffId, attendance.workDate),
          })),
        ];
      },
    }),
    ...
    updateAttendance: builder.mutation<Attendance, UpdateAttendanceInput>({
      async queryFn(input, _queryApi, _extraOptions, baseQuery) {
        ...
        const existingHistories = currentAttendance.histories
          ? currentAttendance.histories.filter(nonNullable).map(cloneExistingHistory)
          : [];
