...
- フィールド構成・バリデーションルール・ペイロードマッピングは `workflow-types.yaml` で宣言的に定義する（詳細は [ワークフロー種別 YAML 設定](./workflow-type-yaml-config) を参照）
...
- カテゴリを追加、削除、改名する場合
  - `workflow-types.yaml`（フィールド・ペイロードマッピング定義）
  - `workflowLabels.ts`（ラベル・表示順・有効無効）
  - フォーム変換（標準フィールドのみなら YAML 変更だけで済む）
  - 管理画面設定
  - 利用者向けガイドの説明
  - 詳細は [ワークフロー種別 YAML 設定](./workflow-type-yaml-config) を参照
- ステータスや承認ロジックを変える場合
  - 一覧、詳細、承認フロー表示
  - 編集可否、取り下げ可否
  - 管理者承認 UI
