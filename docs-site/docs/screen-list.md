...
画面名、URL、関連ガイドを逆引きしたいときの索引として使い、具体的な操作手順や運用判断は各ガイドページを参照してください。

## 使い分け

- サイト構造や共通ガイドの読み順を見たい: [はじめに](./intro)
- 業務全体の流れを見たい: [勤怠・関連申請の業務フロー全体像](./attendance-workflow-overview)
- ロールごとの入口を見たい: [スタッフ向けガイド](./staff/overview)、[管理者向けガイド](./admin/overview)
- 画面遷移を俯瞰したい: [画面遷移マップ（スタッフ向け）](./staff/navigation-map)、[画面遷移マップ（管理者向け）](./admin/navigation-map)
...
| 画面 | 何をする画面か | 関連ガイド | 補足 |
| --- | --- | --- | --- |
| QR 表示 | オフィス端末で QR 打刻の受付画面を表示する | [オフィスモードを運用する](./admin/office-mode-operations) | パス: `/office/qr`。オフィスモード用の特殊運用です。 |
| QR 打刻登録 | QR 経由の打刻を登録する | [オフィスモードを運用する](./admin/office-mode-operations) | パス: `/office/qr/register`。QR からのみ利用する登録画面です。 |
| オフィス入口 | オフィスモードの入口として利用する | [オフィスモードを運用する](./admin/office-mode-operations) | パス: `/office`。実装上は `/office/qr` へ遷移します。 |
...
| 勤怠設定ダイアログ | 勤務時間、半休、出勤モード、特別休暇、欠勤、残業確認、クイック入力をまとめて管理する | [勤怠一覧を確認する](./admin/attendances) | 導線: `/admin/attendances` 右上の `設定` |
| 稼働統計設定 | 稼働統計機能の表示有無を管理する | [設定項目一覧を確認する](./admin/settings-item-list) | パス: `/admin/master/feature_management/attendance_statistics` |
| 打刻画面アナウンス | 打刻画面に表示する案内文を管理する | [打刻補助設定を管理する](./admin/time-recorder-support-settings) | パス: `/admin/master/time_recorder_announcement` |
| 外部リンク設定 | 打刻画面などから参照するリンクを管理する | [打刻補助設定を管理する](./admin/time-recorder-support-settings) | パス: `/admin/master/feature_management/links` |
| 打刻理由設定 | 修正理由や選択肢を管理する | [打刻補助設定を管理する](./admin/time-recorder-support-settings) | パス: `/admin/master/feature_management/reasons` |
| 開発者向け設定 | 内部向けの開発補助設定を確認する | [エクスポートと開発者設定を確認する](./admin/export-and-developer-settings) | パス: `/admin/master/developer` |
| エクスポート | スキーマやデータ出力を実行する | [エクスポートと開発者設定を確認する](./admin/export-and-developer-settings) | パス: `/admin/master/export` |
