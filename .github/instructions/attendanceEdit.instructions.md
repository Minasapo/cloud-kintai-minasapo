...
勤務開始時間・勤務終了時間の入力欄の下には、予め設定された時刻をワンクリック（またはタップ）で反映できるクイック入力用のチップを用意しています。スタッフ・管理者ともに同じ見た目のチップ群で、時刻を選ぶと即時に該当フィールドに `setValue` され、React Hook Form の `shouldDirty` も更新されるので、保存ボタンが活性化されます。

定型入力との違いは次のとおりです。

- 定型入力: 複数フィールド（開始時間／終了時間／休憩／フラグなど）をまとめて更新する大きなアクション。確認ダイアログを経由し、複雑な定型パターンを適用する。
- クイック入力: 勤務開始または勤務終了のいずれかの時刻だけを設定する軽量な操作。フォームの単一フィールドのみを直接書き換えるため、確認なしで使えます。
