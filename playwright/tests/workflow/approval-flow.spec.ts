...
test.describe("ワークフロー承認フロー", () => {
  ...
  test.describe("スタッフユーザー: ワークフロー申請", () => {
    ...
    test("新規ワークフローを申請できること @smoke-test", async ({
      page,
    }, testInfo) => {
    ...
    test("ワークフロー一覧に申請が表示されること @smoke-test", async ({
      page,
    }, testInfo) => {
      if (testInfo.project.name !== "chromium-staff") {
        testInfo.skip();
  ...
  test.describe("管理者ユーザー: 承認・却下操作", () => {
    ...
    test("ワークフロー一覧で申請が確認できること @smoke-test", async ({
      page,
    }, testInfo) => {
    ...
    test("承認操作でステータスが承認済みに変わること @smoke-test", async ({
      page,
    }, testInfo) => {
      if (testInfo.project.name !== "chromium-admin") {
        testInfo.skip();
    ...
    test("却下操作でステータスが却下に変わること @smoke-test", async ({
      page,
    }, testInfo) => {
      if (testInfo.project.name !== "chromium-admin") {
        testInfo.skip();
    ...
    test("承認コメント付き承認ができること", async ({ page }, testInfo) => {
      if (testInfo.project.name !== "chromium-admin") {
        testInfo.skip();
      }
    ...
    test("WorkflowApprovalTimelineが承認フローを表示すること", async ({
      page,
    }, testInfo) => {
      if (testInfo.project.name !== "chromium-admin") {
        testInfo.skip();
