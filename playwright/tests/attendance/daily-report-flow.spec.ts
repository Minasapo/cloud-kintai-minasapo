...
test.describe("日報フロー", () => {
  ...
  test.describe("スタッフ: 日報投稿", () => {
    ...
    test("日報を入力して提出し、提出済みステータスになること @smoke-test", async ({
      page,
    }, testInfo) => {
      ...
      await test.step("新規作成モードへ移行", async () => {
        ...
        // 作成フォームが表示されるまで待機
        await expect(page.getByTestId("daily-report-title-input")).toBeVisible({
          timeout: 10000,
        });
      });
  ...
  test.describe("管理者: 日報確認", () => {
    ...
    test("日報管理画面でカルーセルダイアログを開き詳細を確認できること @smoke-test", async ({
      page,
    }, testInfo) => {
      ...
      await test.step("カルーセルダイアログが開くこと", async () => {
        ...
        if (dialogCount === 0) {
          console.warn("カルーセルダイアログが表示されませんでした — スキップ");
          return;
        }
      ...
      await test.step("ダイアログに日報内容が表示されること", async () => {
        ...
        // タイトル（日報を確認）が表示されること
        await expect(page.getByText("日報を確認")).toBeVisible({
          timeout: 5000,
        });
      });
