...
test.describe("スタッフ管理フロー @smoke-test", () => {
  ...
  test("スタッフ CRUD フロー（作成・編集・削除）@smoke-test", async ({
    page,
  }, testInfo) => {
...
test.describe("スタッフ管理 追加シナリオ", () => {
  ...
  test("勤務形態（シフト勤務 / 固定勤務）の設定", async ({
    ...
    const workTypeSection = page
      .locator("td", { hasText: "勤務形態" })
    ...
    const currentWorkType = await workTypeInput.inputValue().catch(() => "");
